...
export function UserMenu({ user, onLogout }: { user: UserMenuUser; onLogout?: () => void }) {
  ...
  return (
    <div ref={menuRef} className="relative hidden lg:block">
      ...
      {open ? (
        <div role="menu" className="absolute right-0 top-[calc(100%+0.7rem)] z-[80] w-56 rounded-[1.35rem] border border-white/55 bg-white/78 p-2 shadow-[0_24px_80px_rgba(15,23,42,.22)] backdrop-blur-2xl dark:border-white/10 dark:bg-slate-950/82">
          {user.role === "ADMIN" ? <MenuLink href="/admin" label="管理后台" strong /> : null}
          {items.map((item) => <MenuLink key={item.href} href={item.href} label={item.label} />)}
