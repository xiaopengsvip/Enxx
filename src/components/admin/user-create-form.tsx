...
function generateRandomPassword(): string {
  ...
  return `Enxx-${Array.from(bytes).map((value) => alphabet[value % alphabet.length]).join("")}`;
}
...
export function UserCreateForm() {
  ...
  const [created, setCreated] = useState<{ username: string; emailNotice: string } | null>(null);
  function update<K extends keyof FormState>(key: K, value: FormState[K]) { setForm((current) => ({ ...current, [key]: value })); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); setError(""); setNotice(""); setCreated(null);
    ...
    const emailNotice = data.emailNotice === "sent" ? "已发送" : data.emailNotice === "failed" ? "发送失败" : "未发送";
    setNotice(`账号 ${data.user?.username ?? form.username} 已创建。邮件通知：${emailNotice}`);
    setCreated({ username: data.user?.username ?? form.username, emailNotice });
    setForm({ username: "", email: "", displayName: "", role: "USER", initialPassword: "", sendEmailNotify: true });
  }
  return (
    <div className="space-y-5">
      {created ? <Card className="border-emerald-200 bg-emerald-50/80 dark:border-emerald-300/20 dark:bg-emerald-400/10"><p className="text-xl font-black text-emerald-700 dark:text-emerald-100">账号创建成功</p><p className="mt-2 text-sm font-semibold text-emerald-700 dark:text-emerald-100">用户名：{created.username} · 邮件通知：{created.emailNotice}。初始密码不会写入日志，请通过安全渠道告知用户。</p></Card> : null}
      <Card className="space-y-5">
        <form onSubmit={submit} className="grid gap-4 md:grid-cols-2">
          ...
          <div className="md:col-span-2"><Button variant="secondary" onClick={() => update("initialPassword", generateRandomPassword())}>生成随机密码</Button></div>
          ...
          <div className="flex flex-wrap gap-2 md:col-span-2"><Button type="submit" disabled={loading}>{loading ? "创建中..." : "创建用户"}</Button><Button type="button" variant="secondary" onClick={() => setForm({ username: "", email: "", displayName: "", role: "USER", initialPassword: "", sendEmailNotify: true })}>继续创建 / 清空</Button></div>
        </form>
      </Card>
    </div>
  );
