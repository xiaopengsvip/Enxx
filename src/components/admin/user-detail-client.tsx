...
export function UserDetailClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/users/${id}`).then((res) => res.json()).then((data) => { if (!data.ok) throw new Error(data.message ?? "用户详情加载失败"); setUser(data.user); setStats(data.stats); setEmailLogs(data.emailLogs ?? []); }).catch((err: unknown) => setError(err instanceof Error ? err.message : "用户详情加载失败"));
  }, [id]);
