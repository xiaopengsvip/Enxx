...
export default function AccountPage() {
  ...
  const load = useCallback(async () => {
    ...
    const logsResponse = await fetch("/api/account/login-logs"); if (logsResponse.ok) { const logsData = await logsResponse.json(); setLoginLogs(logsData.items ?? []); }
    setLoading(false);
  }, []);
  useEffect(() => {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="space-y-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-black">登录安全</h2><p className="mt-1 text-sm font-semibold text-slate-500 dark:text-slate-300">最近登录 IP、地区、设备、浏览器和系统。</p></div><Link href="/account/security"><Button variant="secondary">查看登录记录</Button></Link></div><div className="grid gap-3 md:grid-cols-3"><Info label="最近登录 IP" value={loginLogs[0]?.ip ?? "未记录"} /><Info label="最近登录地区" value={loginLogs[0]?.location ?? "未知地区"} /><Info label="最近设备" value={[loginLogs[0]?.device, loginLogs[0]?.browser, loginLogs[0]?.os].filter(Boolean).join(" / ") || "未记录"} /></div><div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.18em] text-slate-400"><tr><th className="p-3">IP</th><th className="p-3">地区</th><th className="p-3">设备</th><th className="p-3">浏览器</th><th className="p-3">系统</th><th className="p-3">时间</th></tr></thead><tbody>{loginLogs.slice(0, 5).map((log) => <tr key={log.id} className="border-t border-slate-100 dark:border-white/10"><td className="p-3 font-mono text-xs">{log.ip ?? "unknown"}</td><td className="p-3">{log.location || "未知地区"}</td><td className="p-3">{log.device ?? "—"}</td><td className="p-3">{log.browser ?? "—"}</td><td className="p-3">{log.os ?? "—"}</td><td className="p-3">{new Date(log.createdAt).toLocaleString()}</td></tr>)}</tbody></table>{!loginLogs.length ? <p className="py-4 text-sm font-semibold text-slate-500">暂无登录记录。</p> : null}</div></Card>
