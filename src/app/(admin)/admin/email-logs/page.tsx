...
export default function AdminEmailLogsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="overflow-x-auto"><table className="w-full min-w-[900px] text-left text-sm"><thead className="text-xs uppercase tracking-[0.18em] text-slate-400"><tr><th className="p-3">Provider</th><th className="p-3">发件人</th><th className="p-3">收件人</th><th className="p-3">类型</th><th className="p-3">主题</th><th className="p-3">状态</th><th className="p-3">时间</th><th className="p-3">错误</th><th className="p-3">messageId</th></tr></thead><tbody>{logs.map((log) => <tr key={log.id} className="border-t border-slate-100 dark:border-white/10"><td className="p-3">{log.providerKey ?? "—"}</td><td className="p-3">{log.from ?? "—"}</td><td className="p-3">{log.to}</td><td className="p-3 font-mono text-xs">{log.type}</td><td className="p-3 font-bold">{log.subject}</td><td className="p-3"><span className={["success", "SENT"].includes(log.status) ? "rounded-full bg-emerald-100 px-3 py-1 font-black text-emerald-700" : log.status === "skipped" ? "rounded-full bg-amber-100 px-3 py-1 font-black text-amber-700" : "rounded-full bg-rose-100 px-3 py-1 font-black text-rose-700"}>{log.status}</span></td><td className="p-3">{new Date(log.createdAt).toLocaleString()}</td><td className="p-3 text-rose-500">{log.error ?? "—"}</td><td className="p-3 font-mono text-xs">{log.messageId ?? "—"}</td></tr>)}</tbody></table></Card>
    </div>
  );
}
